...
import { MAX_FLAT_NUMBER, MAX_HOUSE_NUMBER, MIN_FLAT_NUMBER, Region, UserDataType } from '../../constants';
import { getUserData } from '../file.service';
import { getPrNum, getPrRecord } from '../random.service';
import { MIN_HOUSE_NUMBER } from './../../constants/index';
import seedrandom from 'seedrandom';

export class ByUserGenerator extends UserGenerator {
  ...
  protected generateMiddleName(genPR: seedrandom.PRNG): string {
    return getPrRecord(this.middleNames, genPR).trim() + (genPR() < 0.5 ? 'ўна' : 'віч');
  }
  ...
  protected generateAddress(genPR: seedrandom.PRNG): string {
    ...
    const flat = city.slice(0, 1) == 'г' ? 'кв. ' + getPrNum(MIN_FLAT_NUMBER, MAX_FLAT_NUMBER, genPR) + ', ' : '';
